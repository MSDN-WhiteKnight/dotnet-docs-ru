...
|Отобразить столбец фиксированной ширины, например столбец значка или идентификатора.|Задайте <xref:System.Windows.Forms.DataGridViewColumn.AutoSizeMode%2A> для <xref:System.Windows.Forms.DataGridViewAutoSizeColumnMode.None> и <xref:System.Windows.Forms.DataGridViewColumn.Resizable%2A> для <xref:System.Windows.Forms.DataGridViewTriState.False> для столбца. Инициализируйте его ширину, задав <xref:System.Windows.Forms.DataGridViewColumn.Width%2A> свойства или путем вызова элемента управления <xref:System.Windows.Forms.DataGridView.AutoResizeColumn%2A> метод после заполнения элемента управления данными.|  
