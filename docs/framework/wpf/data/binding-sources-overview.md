...
|--------------------|-----------------|  
|CLR объекты|Можно осуществить привязку к открытым свойствам, подсвойствам, а также индексаторам любого объекта CLR. Обработчик привязки использует отражение CLR для получения значений свойств. Кроме того, объекты, реализующие <xref:System.ComponentModel.ICustomTypeDescriptor> или имеют зарегистрированные <xref:System.ComponentModel.TypeDescriptionProvider> также работают с обработчиком привязки.<br /><br /> Дополнительные сведения о том, как реализовать класс, который можно использовать в качестве источника привязки, см. в разделе [Использование класса в качестве источника привязки](#classes).|  
...
### <a name="providing-change-notifications"></a>Предоставление уведомлений об изменениях  
 Если используется какая-либо <xref:System.Windows.Data.BindingMode.OneWay> или <xref:System.Windows.Data.BindingMode.TwoWay> привязки (так как требуется вашей UI обновление при динамическом изменении свойств источника привязки), необходимо реализовать подходящий механизм изменениях свойств уведомлений. Рекомендуемый механизм предназначен для CLR или динамического класса для реализации <xref:System.ComponentModel.INotifyPropertyChanged> интерфейс. Дополнительные сведения см. в разделе [Реализация уведомления об изменении свойств](how-to-implement-property-change-notification.md).  
...
-   Если вы хотите создать объект в XAML, класс должен иметь конструктор по умолчанию. В некоторых [!INCLUDE[TLA2#tla_net](../../../../includes/tla2sharptla-net-md.md)] языки, такие как C#, конструктор по умолчанию может быть создан для вас.  
...
-   Всегда можно осуществить привязку к свойствам зависимостей.  
 ...
 Требование разрешения для привязки XML то же. В песочнице с частичным доверием <xref:System.Windows.Data.XmlDataProvider> завершается ошибкой, если он не имеет разрешения на доступ к указанным данным.  
