...
## <a name="mapping-to-custom-classes-and-assemblies"></a>Сопоставление пользовательских классов и сборок  
 ...
 `assembly=` Сборка, содержащая некоторые или все указанные [!INCLUDE[TLA2#tla_clr](../../../../includes/tla2sharptla-clr-md.md)] пространства имен. Это значение обычно содержит только имя сборки, но не путь к ней, и не включает расширение имени файла (например, EXE или DLL). Путь к этой сборке должен быть задан в качестве ссылки проекта в файле проекта, который содержит код XAML, который следует сопоставить. Чтобы включить управление версиями и подписи строгого имени, `assembly` значение может быть строкой в соответствии с определением <xref:System.Reflection.AssemblyName>, а не простым именем строки.  
