...
Взаимодействия между WPF и Windows Forms должна иметь обработки ввода сочетания обеих технологий. В этом разделе описывается, как реализовать эти технологии, клавиатуры и обработки сообщений с целью обеспечить корректное взаимодействие в гибридных приложениях.  
...
## <a name="modeless-forms-and-dialog-boxes"></a>Безрежимные формы и диалоговые окна  
 ...
 Вызовите <xref:System.Windows.Forms.Integration.ElementHost.EnableModelessKeyboardInterop%2A> метод <xref:System.Windows.Forms.Integration.ElementHost> элемента управления, чтобы открыть немодальный WPF странице в Windows Forms-приложения на основе.  
  
## <a name="windowsformshost-keyboard-and-message-processing"></a>WindowsFormsHost-клавиатура и обработки сообщений  
 Если они размещаются в WPF-приложения, на основе Windows Forms клавиатуры и обработка состоит из следующих сообщений:  
...
-   <xref:System.Windows.Forms.Integration.WindowsFormsHost> Класс создает суррогат Windows Forms цикл обработки сообщений, чтобы убедиться, что обычные Windows Forms происходит обработка клавиатуры.  
...
### <a name="acquiring-messages-from-the-wpf-message-loop"></a>Получение сообщений из цикла обработки сообщений WPF  
 ...
 Взаимодействия реализация обрабатывает <xref:System.Windows.Interop.ComponentDispatcher.ThreadFilterMessage?displayProperty=nameWithType> событие, которое позволяет Windows Forms элементы управления для обработки сообщений перед WPF элементов управления.  
  
### <a name="surrogate-windows-forms-message-loop"></a>Цикл обработки сообщений суррогат Windows Forms  
 По умолчанию <xref:System.Windows.Forms.Application?displayProperty=nameWithType> класс содержит цикл основного сообщения для Windows Forms приложений. Во время взаимодействия Windows Forms сообщение, цикл не обрабатывает сообщения. Таким образом эта логика должна быть воспроизведена. Обработчик <xref:System.Windows.Interop.ComponentDispatcher.ThreadFilterMessage?displayProperty=nameWithType> событий выполняет следующие действия:  
...
### <a name="ikeyboardinputsink-implementation"></a>Реализация IKeyboardInputSink  
 ...
 По умолчанию <xref:System.Windows.Interop.HwndHost> возвращает `false` для его <xref:System.Windows.Interop.HwndHost.System%23Windows%23Interop%23IKeyboardInputSink%23TabInto%2A> реализации. Это предотвращает переход из WPF управления Windows Forms элемента управления.  
...
1. Находит первый или последний Windows Forms управления, содержащийся в <xref:System.Windows.Forms.Integration.WindowsFormsHost> и может получать фокус. Выбор элемента управления зависит от обхода сведений.  
...
## <a name="elementhost-keyboard-and-message-processing"></a>ElementHost-клавиатура и обработки сообщений  
 Если они размещаются в Windows Forms приложения, WPF клавиатуры и обработка состоит из следующих сообщений:  
...
-   Если клавиша ALT нажата, сообщение является WM_SYSCHAR. Windows Forms не обрабатывает это сообщение с помощью <xref:System.Windows.Forms.Control.IsInputChar%2A> метод. Таким образом <xref:System.Windows.Forms.Control.ProcessMnemonic%2A> переопределяется метод для запроса WPF <xref:System.Windows.Input.AccessKeyManager> для зарегистрированному сочетанию клавиш. При обнаружении зарегистрированному сочетанию клавиш <xref:System.Windows.Input.AccessKeyManager> обрабатывает его.  
