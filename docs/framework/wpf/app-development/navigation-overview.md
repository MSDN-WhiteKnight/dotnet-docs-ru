...
### <a name="implementing-a-page"></a>Реализация страницы  
 ...
 Объект <xref:System.Windows.Controls.Page> , реализованный в XAML имеет разметки `Page` качестве корневого элемента и требует WPFXML объявление пространства имен. `Page` Элемент содержит содержимое, которое вы хотите перейти и отобразить. Добавления содержимого задается `Page.Content` элемента свойства, как показано в следующем примере разметки.  
